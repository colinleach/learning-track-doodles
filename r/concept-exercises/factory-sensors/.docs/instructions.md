...
- When the sensor is working, if the temperature exceeds 500°C, you should halt with a message that includes the temperature.
- Otherwise, all is well, so send the message `"temperature check passed: t °C"`, where `t` is the temperature.
